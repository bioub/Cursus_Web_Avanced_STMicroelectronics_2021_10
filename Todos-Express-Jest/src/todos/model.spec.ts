import { TodoArrayService } from './model';
...
describe('TodoArrayService class', () => {
  ...
  describe('method find', () => {
    it('should resolve an array of todos', async () => {
      ...
      expect(todos).toEqual([{ id: 1, title: 'ABC', completed: false }]);
    });
  });
  ...
  describe('method create', () => {
    it('should return number of completed todos', async () => {
      ...
      expect(newTodo).toEqual({ title: 'XYZ', completed: false, id: 2 }); // deepEqual
      expect(newTodo).not.toBe(todoToCreate); // !==
    });
  });
});
